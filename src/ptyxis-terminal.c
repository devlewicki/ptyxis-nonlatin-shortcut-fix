...
static gboolean
ptyxis_terminal_keycode_matches_accel (PtyxisTerminal     *self,
                                       GdkEvent           *event,
                                       guint               keycode,
                                       GdkModifierType     state,
                                       const char         *shortcut_prop)
{
  static const GdkModifierType mask = GDK_CONTROL_MASK |
                                      GDK_SHIFT_MASK |
                                      GDK_ALT_MASK |
                                      GDK_SUPER_MASK |
                                      GDK_HYPER_MASK |
  ...
  g_object_get (self->shortcuts, shortcut_prop, &accelerator, NULL);

  if (accelerator == NULL || accelerator[0] == 0)
    return FALSE;

  if (!gtk_accelerator_parse (accelerator, &keyval, &modifiers))
    return FALSE;

  consumed = gdk_key_event_get_consumed_modifiers (event);

  if ((modifiers & ~consumed & mask) != (state & ~consumed & mask))
    return FALSE;

  if (gdk_display_map_keyval (gdk_event_get_display (event), keyval, &keys, &n_keys))
    {
      for (int i = 0; i < n_keys && !matched; i++)
        {
                                        ...
                                        GtkEventController *controller)
{
  ...
  if (ptyxis_terminal_keycode_matches_accel (self, event, keycode, state, "copy-clipboard"))
    {
      gtk_widget_activate_action (GTK_WIDGET (self), "clipboard.copy", NULL);
      return TRUE;
    }

  if (ptyxis_terminal_keycode_matches_accel (self, event, keycode, state, "paste-clipboard"))
    {
      gtk_widget_activate_action (GTK_WIDGET (self), "clipboard.paste", NULL);
      return TRUE;
    }
